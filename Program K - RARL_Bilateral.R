...
## Function that calculates the ARL EWMA.Change as needed. 
...
k2a=3.88
#EWMA Limits 
LCL1a=u0-k1a*((lambda1a/(2-lambda1a))^0.5)*(sd0/(n^0.5))
UCL1a=u0+k1a*((lambda1a/(2-lambda1a))^0.5)*(sd0/(n^0.5))
c=log(sd0^2)-1/(n-1)-1/(3*(n-1)^2)+2/(15*(n-1)^4)
d=2/(n-1)+2/((n-1)^2)+4/(3*(n-1)^3)-16/(15*(n-1)^5)
LCL2a=c-k2a*(d*lambda2a/(2-lambda2a))^0.5
UCL2a=c+k2a*(d*lambda2a/(2-lambda2a))^0.5

#Function that calculates the ARL EWMA.Change as needed.
Inte_EWMAa <- function(params3) {
  u=params3[1]
  ss=params3[2]
  Result<-matrix(0, runs, 1)
  ...
  for(i in 1:runs){
    ...
    while(s<1){
      ...
      R=mean(RR)
      V=var(RR)
      T=lambda1a*R+(1-lambda1a)*ua
      W=lambda2a*log(V)+(1-lambda2a)*sa
      
      if (T>UCL1a | T<LCL1a |W>UCL2a |W<LCL2a){
        D<-rbind(D,1)
        s=s+1
      }else{
        D<-rbind(D,1)
...
Saida=matrix(0,corridas,4)
for (i in 1:corridas){
 a1=RR1[i]
 a2=RR2[i]
 v=c(a1,a2)
 #Change as needed.
 P1a=Inte(v)/Inte_EWMA(v)
 P2a=Inte_double(v)/Inte_EWMA(v)
 P3a=Inte_EWMA(v)/Inte_EWMA(v)
 P4a=Inte_EWMAa(v)/Inte_EWMA(v)
  Saida[i,1]=P1a
  Saida[i,2]=P2a
  Saida[i,3]=P3a
  Saida[i,4]=P4a
...
RARL_MC1=(mean(Saida[,1]))
RARL_MC2=(mean(Saida[,2]))
RARL_MC3=(mean(Saida[,3]))
RARL_MC4=(mean(Saida[,4]))


cat("Monte Carlo Simulation","\n")
cat("RARL-Xb-S2 MC=",RARL_MC1,"\n")
cat("RARL-Xb-S2 Klein MC=",RARL_MC2,"\n") 
cat("RARL-EWMA Xb-S2 MC=",RARL_MC3,"\n") 
cat("RARL-EWMAa Xb-S2 MC=",RARL_MC4,"\n") 
toc()